...
import re
from netcutterform.models import *
import json
...
def index_view(request):
    ...
    return render(request, 'netcutterform/index.html', response)
...
def upload_graph(request):
    ...
    '''
    response = dict()
    template = 'netcutterform/404.html'

    if request.method == "POST":
        template = 'netcutterform/gexplorer.html'
        experiments = Experiment.all_from_database()
        exp_names = [ experiment.identifier for experiment in experiments ]
        response['experiments'] = exp_names
        # Upload graph
...
def gene_explorer(request):
    ...
    return render(request, 'netcutterform/gexplorer.html', response)
...
def get_properties(request):
    ...
    if request.is_ajax():
        ...
        if 'gene' in request.GET:
            ...
            template = 'netcutterform/gene_properties.html'
            ...
            template = 'netcutterform/int_properties.html'
            response['interaction'] = int_obj
        return render(request, template, response)
    else:
        return render(request, 'netcutterform/404.html')
    ...
    if request.is_ajax():
        ...
        if 'gene' in request.GET and 'level' in request.GET and 'exp' in request.GET:
            ...
            if request.GET['exp'] and request.GET['exp'] != "None":
                # "None" string needed because javascript -> ajax -> add_neighbours
                exp_id = request.GET['exp']
            else:
                exp_id = None
...
def pathway_explorer(request):
    '''
    Find Pathways from your genes to specific levels of the netcutter graph
    '''
    ...
    return render(request, 'netcutterform/pexplorer.html', response)
...
def shortest_path(request):
    ...
    if response['error'] is False:
        ...
        for inter in allpaths[0].interactions:
            print(inter.physical_interaction)
        experiment = Experiment(exp_id).check()
...
def tutorial(request):
    ...
    Tutorial view
    '''
    return render(request, 'netcutterform/tutorial.html')
...
def data(request):
    ...
    return render(request, 'netcutterform/data.html', response)
...
def about(request):
    ...
    About view
    '''
    return render(request, 'netcutterform/about.html')
...
def handler404(request):
    ...
    """
    response = render_to_response('netcutterform/404.html', {},
...
def handler500(request):
    ...
    """
    response = render_to_response('netcutterform/500.html', {},
                                  context_instance=RequestContext(request))
